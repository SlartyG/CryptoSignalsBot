...
## Шаг 3. Оплата (если нужна подписка)

Нужен хотя бы один провайдер. Пользователь выбирает способ оплаты после выбора валюты.
...
1. Откройте **@xrocket** → **Rocket Pay** → **Create App** → **API token**.
2. Токен — `XROCKET_PAY_TOKEN`. Документация: [pay.xrocket.exchange/api](https://pay.xrocket.exchange/api/).

Без токенов бот запустится, но оплата будет недоступна.  
